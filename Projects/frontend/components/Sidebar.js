...
import { FiHome, FiUsers, FiBook, FiUserCheck, FiCalendar, FiLayers, FiClock } from 'react-icons/fi'
  ...
  return (
    <aside className={`${collapsed ? 'w-16' : 'w-60'} bg-white dark:bg-slate-800 border-r h-full transition-width duration-150 flex-shrink-0`}>
      ...
      <nav className="mt-4 px-2 flex flex-col gap-1">
        ...
        <Link href="/attendance" className="flex items-center gap-3 p-2 rounded hover:bg-gray-100 dark:hover:bg-slate-700 text-slate-900 dark:text-slate-100"><FiClock className="text-lg" />{!collapsed && <span>{t('sidebar.attendance', 'Attendance')}</span>}</Link>
